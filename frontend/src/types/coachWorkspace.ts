...
export interface CoachSession { id:number; member_id:number; member_name:string; started_at:string; completed_at:string|null; status:string; workout_name:string; source:'member'; }
export interface CoachSessionHistoryItem { id:number; member_id:number; workout_id:number|null; assignment_id?:number; schedule_id?:number; started_at:string; completed_at:string|null; status:string; notes:string|null; workout_name:string; workout_description:string|null; setSummary:{total:number;completed:number}|null; blockedReason:null; source:'member'; }
export interface CoachSessionDetail extends CoachSessionHistoryItem { exerciseSnapshot:unknown[]; }
export interface CoachProgressSession { id:number; started_at:string; completed_at?:string|null; ended_at?:string|null; status:string; workout_name?:string; program_name?:string; day_title?:string; total_duration_seconds?:number|null; scheduled_date?:string; source:'member'; }
