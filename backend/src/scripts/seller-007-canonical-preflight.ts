...
(SELECT COUNT(*) FROM sys.indexes WHERE object_id IN(OBJECT_ID(N'dbo.Products'),OBJECT_ID(N'dbo.ProductVariants'),OBJECT_ID(N'dbo.Inventory'),OBJECT_ID(N'dbo.Shops')))marketplace_indexes`);
console.log(`SELLER007_PREFLIGHT ${JSON.stringify(result.recordset[0])}`);}finally{await closePool();}}
