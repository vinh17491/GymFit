...
import { clearRefreshTokenCookie, readRefreshTokenCookie, setRefreshTokenCookie } from '../../utils/refreshCookie';
...
async function createSession(user:SessionUser,family=crypto.randomUUID(),transaction?:sql.Transaction) {
  ...
  const values={userId:user.id,hash:tokenHash(refreshToken),family,expiresAt:new Date(Date.now()+config.jwt.refreshLifetimeMs)};
...
export async function register(req:Request,res:Response,next:NextFunction) {
  ...
  try {
    ...
    sendSuccess(res,{user:publicUser(user),accessToken:tokens.accessToken},'Registered',201);
  } catch(error){
...
export async function login(req:Request,res:Response,next:NextFunction) {
  try { const email=normalizeEmail(req.body.email);const result=await query(`SELECT id,email,name,phone,role,referral_code,avatar_url,is_active,token_version,password FROM dbo.Users WHERE LOWER(email)=@email`,{email}); const row=result.recordset[0];const valid=await bcrypt.compare(req.body.password,row?.password||DUMMY_HASH); if(!row||!valid||!row.is_active) throw new AppError(401,'Invalid email or password');await query('UPDATE dbo.Users SET last_login_at=GETDATE() WHERE id=@id',{id:row.id}); const user=sessionUser(row);const tokens=await createSession(user);setRefreshTokenCookie(res,tokens.refreshToken);sendSuccess(res,{user:publicUser(user),accessToken:tokens.accessToken},'Login successful'); }
  catch(error){next(error);}
}
export async function logout(req:Request,res:Response,next:NextFunction) { try{await query('UPDATE dbo.AuthSessions SET revoked_at=COALESCE(revoked_at,SYSUTCDATETIME()) WHERE id=@id AND user_id=@uid',{id:req.user!.sessionId,uid:req.user!.userId});clearRefreshTokenCookie(res);sendSuccess(res,null,'Logged out');}catch(error){next(error);} }
...
export async function refreshToken(req:Request,res:Response,next:NextFunction) {
  const raw=readRefreshTokenCookie(req); if(!raw){clearRefreshTokenCookie(res);return next(new AppError(400,'Refresh token required'));}
  ...
  try {
    ...
    const row=found.recordset[0]; if(!row){clearRefreshTokenCookie(res);await tx.rollback();complete=true;throw new AppError(401,'Invalid refresh token');}
    if(row.revoked_at){clearRefreshTokenCookie(res);await new sql.Request(tx).input('family',sql.UniqueIdentifier,row.token_family).query('UPDATE dbo.AuthSessions SET revoked_at=COALESCE(revoked_at,SYSUTCDATETIME()) WHERE token_family=@family');await tx.commit();complete=true;throw new AppError(401,'Refresh token replay detected');}
    if(!row.is_active||new Date(row.expires_at)<=new Date()){clearRefreshTokenCookie(res);await new sql.Request(tx).input('id',sql.BigInt,row.session_id).query('UPDATE dbo.AuthSessions SET revoked_at=COALESCE(revoked_at,SYSUTCDATETIME()) WHERE id=@id');await tx.commit();complete=true;throw new AppError(401,'Invalid refresh token');}
    const nextRaw=crypto.randomBytes(48).toString('base64url'); const inserted=await new sql.Request(tx).input('uid',sql.Int,row.user_id).input('hash',sql.Char(64),tokenHash(nextRaw)).input('family',sql.UniqueIdentifier,row.token_family).input('expires',sql.DateTime2,new Date(Date.now()+config.jwt.refreshLifetimeMs)).query('INSERT dbo.AuthSessions(user_id,refresh_token_hash,token_family,expires_at) OUTPUT INSERTED.id VALUES(@uid,@hash,@family,@expires)'); const nextId=Number(inserted.recordset[0].id);
    await new sql.Request(tx).input('old',sql.BigInt,row.session_id).input('next',sql.BigInt,nextId).query('UPDATE dbo.AuthSessions SET revoked_at=SYSUTCDATETIME(),last_used_at=SYSUTCDATETIME(),replaced_by_session_id=@next WHERE id=@old AND revoked_at IS NULL'); await tx.commit();complete=true; const user=sessionUser(row); setRefreshTokenCookie(res,nextRaw); sendSuccess(res,{accessToken:accessToken(user,nextId),user:publicUser(user)});
