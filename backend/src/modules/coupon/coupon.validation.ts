...
export const createCouponSchema = z.object({
  code: z.string().trim().min(3).max(50), type: z.enum(['fixed','percentage','free_trial','first_purchase','referral','flash_sale']),
...
export const applyCouponSchema = z.object({ code: z.string().trim().min(3).max(50), plan_id: z.number().int().positive() }).strict();
