...
export const refundsService={
  ...
  async update(id:number,adminId:number,input:{status:RefundStatus;reason:string;externalReference?:string}){
    ...
    try{
      ...
      await tx.request().input("id",sql.BigInt,id).input("status",sql.NVarChar(20),input.status).input("currentStatus",sql.NVarChar(20),current.status).input("reference",sql.NVarChar(255),input.externalReference?.trim()||null).query("UPDATE dbo.Refunds SET status=@status,external_reference=COALESCE(@reference,external_reference),completed_at=CASE WHEN @status=N'COMPLETED' THEN SYSUTCDATETIME() ELSE NULL END,updated_at=SYSUTCDATETIME() WHERE id=@id AND status=@currentStatus; IF @@ROWCOUNT<>1 THROW 51013,'Concurrent refund status change detected.',1");
